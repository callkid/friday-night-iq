...
(async()=>{
  ...
  await page.click('.nav button[data-screen="settings"]');await page.waitForSelector('#themeToggle',{state:'attached'});if(await page.locator('#themeToggle').isChecked())await page.locator('.themeSwitch').click();await page.waitForFunction(()=>document.documentElement.getAttribute('data-theme')==='light');
  ...
  console.log('BOX E2E PASS: compact primary controls, formation reset/personnel carry, 2nd&2 false-start -> 2nd&7, no-scroll penalty, Run Type stability, semantic colors, visible explosive thresholds, light mode, quick-stat drilldown, 1st&10, live filter click');
  await browser.close();
