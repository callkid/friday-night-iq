...
run_shell "Adaptive viewport contract" 'grep -q "1475x668" qa-browser-responsive.js && grep -q "1920" qa-browser-responsive.js && grep -q "2560" qa-browser-responsive.js && grep -q "grid-auto-rows:max-content" adaptive-layout.css && grep -q "max-height:700px" adaptive-layout.css && grep -q "min-width:1600px" adaptive-layout.css && grep -q "overflow-y:auto" adaptive-layout.css && grep -q "position:static!important" adaptive-layout.css && grep -q "shortSituationEdit" box-ui-v19.js && grep -q "boxDockedSummary" box-ui-v19.js'
