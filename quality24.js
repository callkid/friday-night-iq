...
function install(A,root){
 ...
 function ensureOption(select,value,label){if(!select||[].some.call(select.options,function(o){return o.value===value;}))return;var o=d.createElement('option');o.value=value;o.textContent=label||value;select.insertBefore(o,select.options.length>1?select.options[1]:null);}
 ...
 function ensureConceptQuick(){var select=$('conceptFamily');if(!select||$('q24ConceptQuick'))return;var host=select.parentElement,bar=d.createElement('div');bar.id='q24ConceptQuick';bar.className='q24QuickRow q24ConceptQuick';[['RUN','Run'],['RPO','RPO'],['SHORT','Short Game'],['DROP','Drop Back'],['SCREEN','Screen'],['PA','Play Action']].forEach(function(x){var b=makeQuickButton(x[0],x[1]);b.onclick=function(){select.value=x[1];fire(select);syncConceptQuick();};bar.appendChild(b);});host.insertBefore(bar,select);select.classList.add('q24ConceptFallback');select.addEventListener('change',syncConceptQuick);syncConceptQuick();}
 function syncConceptQuick(){var select=$('conceptFamily'),bar=$('q24ConceptQuick');if(!select||!bar)return;bar.querySelectorAll('button').forEach(function(b){b.classList.toggle('on',b.dataset.value===select.value);});}
 ...
 function ensureLive(){resetTouchedOnNewPlay();ensureCoverageQuick();ensureMotionQuick();ensureConceptQuick();ensureHurry();ensureCapture();syncCoverageQuick();syncMotionQuick();syncConceptQuick();renderCapture();}
