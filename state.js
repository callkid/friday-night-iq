...
function stampNext(p,n){p.next={quarter:n.quarter,down:n.down,distance:n.distance,fieldSide:n.fieldSide,yardLine:n.yardLine,drive:n.drive,possessionEnded:n.possessionEnded,reason:n.reason};return p}
function applyNextState(A,p,n){A.state.current={quarter:n.quarter,down:n.down,distance:n.distance,fieldSide:n.fieldSide,yardLine:n.yardLine};A.state.drive=n.drive||A.state.drive;A.state.awaitingPossessionStart=!!n.possessionEnded;if(n.reason==='Touchdown'&&p.tags.indexOf('Touchdown')<0)p.tags.push('Touchdown')}
...
A.applyPlay=function(p){A.state.context={formation:p.formation||'NA',personnel:p.personnel&&p.personnel!=='NA'?p.personnel:''};var n=E.nextSituation(p);stampNext(p,n);applyNextState(A,p,n);A.state.plays.push(p);A.save('play');if(A.durabilitySnapshot)A.durabilitySnapshot('play');return n};
A.replaceLastPlay=function(p){if(!A.state.plays.length)return null;var old=A.state.plays[A.state.plays.length-1];p.number=old.number;p.drive=old.drive;p.quarter=old.quarter;p.down=old.down;p.distance=old.distance;p.fieldSide=old.fieldSide;p.yardLine=old.yardLine;p.tags=Array.isArray(p.tags)?p.tags:[];var n=E.nextSituation(p);stampNext(p,n);A.state.context={formation:'NA',personnel:p.personnel&&p.personnel!=='NA'?p.personnel:''};applyNextState(A,p,n);A.state.plays[A.state.plays.length-1]=p;A.save('edit-last');if(A.durabilitySnapshot)A.durabilitySnapshot('edit-last');return n};
