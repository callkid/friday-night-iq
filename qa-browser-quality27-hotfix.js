...
async function metric(page){
 return page.locator('#save').evaluate(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el),h=getComputedStyle(document.documentElement);return{height:r.height,font:parseFloat(s.fontSize),zoom:h.zoom||'1',scrollW:document.documentElement.scrollWidth,innerW:innerWidth};});
}
async function setSize(page,size){
 await page.click('#settingsNav');await page.waitForSelector('#settings.on');await page.click('[data-q27-size="'+size+'"]');await page.click('[data-screen="live"]');await page.waitForSelector('#live.on');await page.waitForTimeout(100);
}
...
(async()=>{
 ...
 const standard=await metric(page);await page.screenshot({path:'qa-screenshots/q27-size-standard-1475.png',fullPage:true});
 await setSize(page,'large');const large=await metric(page);await page.screenshot({path:'qa-screenshots/q27-size-large-1475.png',fullPage:true});
 ...
 assert(large.scrollW<=large.innerW+2,'Large mode has horizontal page overflow: '+JSON.stringify(large));
 await setSize(page,'xl');const xl=await metric(page);await page.screenshot({path:'qa-screenshots/q27-size-xl-1475.png',fullPage:true});
 ...
 assert(xl.scrollW<=xl.innerW+2,'Extra Large mode has horizontal page overflow: '+JSON.stringify(xl));
 ...
 await page.screenshot({path:'qa-screenshots/q27-holding-repeat-preview.png',fullPage:true});
 ...
 console.log('QUALITY27 HOTFIX BROWSER PASS: Standard/Large/Extra Large visibly resize live controls without zoom or horizontal overflow, accepted Holding repeats the down, and generic accepted live-ball penalties no longer default to consuming the down');
