...
(async()=>{
 ...
 await page.reload({waitUntil:'networkidle'});await page.waitForSelector('#q27Settings',{state:'attached'});assert(await page.locator('html').evaluate(el=>el.classList.contains('fniqQ27SizeLarge')),'Control size did not survive reload');await page.click('#settingsNav');await page.waitForSelector('#settings.on');assert(await page.locator('#q27Settings').isVisible(),'Quality27 Settings card did not render after reload');await page.click('[data-screen="live"]');await page.waitForSelector('#live.on');assert(await page.locator('#q27Pins-formation button',{hasText:formationName}).isVisible(),'Formation pin did not survive reload');assert.equal((await page.evaluate(()=>window.FNIQ.state.plays.length)),1,'Quality27 preferences disturbed saved plays');
