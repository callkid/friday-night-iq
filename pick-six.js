...
function install(A,root){
 ...
 var oldReplace=A.replaceLastPlay;if(oldReplace)A.replaceLastPlay=function(p){var old=A.state.plays[A.state.plays.length-1],current=Number(A.state.score.them)||0,remove=Number(old&&old.scoreAutoOpponentPoints)||0;A.state.score.them=revised(current,remove,0);var n=oldReplace.call(A,p);if(!n){A.state.score.them=current;return n}var saved=A.state.plays[A.state.plays.length-1],pts=p&&p.pickSix?6:0;A.state.score.them=revised(A.state.score.them,0,pts);if(saved){saved.pickSix=!!p.pickSix;saved.scoreAutoOpponentPoints=pts;saved.scoreAfter={us:Number(A.state.score.us)||0,them:Number(A.state.score.them)||0}}A.save('edit-pick-six-score');return n};
