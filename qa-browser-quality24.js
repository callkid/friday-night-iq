...
(async()=>{
  ...
  await page.waitForSelector('#live.on');await page.waitForSelector('#q27Pins-coverage');await page.waitForSelector('#q27Pins-motion');await page.waitForSelector('#q24Hurry');await page.waitForSelector('#q24Capture');await page.waitForSelector('#q24AttackQuick',{state:'attached'});
  assert(await page.locator('#q27Pins-coverage button[data-value="Cover 3"]').isVisible(),'pre-snap coverage quick buttons missing');
  ...
  await page.click('#q27Pins-coverage button[data-value="Cover 3"]');assert.equal(await page.inputValue('#coverage'),'Cover 3');
  await page.click('[data-group="box"] [data-v="6"]');
  await page.click('#q27Pins-motion button[data-value="No Motion"]');assert.equal(await page.inputValue('#motion'),'No Motion','No Motion must be stored explicitly, not as N/A');
  ...
  assert(!/Look\s+\d+\/8/i.test(await page.locator('#q24CaptureCount').textContent()),'Quality26/27 must use action-oriented capture copy');
  ...
  for(const s of ['#speedHashButtons','#formation','#personnel','[data-group="front"]','[data-group="safeties"]','#q27Pins-coverage','[data-group="box"]','#motion','[data-group="playType"]','#yards','#save'])assert(await page.locator(s).first().isVisible(),'hurry-up hid essential '+s);
  ...
  assert.equal(await page.locator('#q27Pins-motion button[data-value="H-Jet"]').count(),0,'recent Motion must not become an unrequested duplicate quick button');
  ...
  for(const yards of ['6','7']){
    ...
    await page.waitForSelector('#q27Pins-runType button[data-value="Inside Zone"]',{state:'visible'});await page.click('#q27Pins-runType button[data-value="Inside Zone"]');
